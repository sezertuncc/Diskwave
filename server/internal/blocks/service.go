...
// Uses a separate "block_" prefix to avoid colliding with path-based file keys.
func (s *Service) BlockExists(ctx context.Context, hash string) (bool, error) {
	return s.storage.Exists(ctx, blockHashKey(hash))
}

// blockHashKey namespaces content-addressed blocks away from path-keyed files.
func blockHashKey(hash string) string { return "block_" + hash }
...
func (s *Service) WriteToStaging(path string, r io.Reader) (string, error) {
	tmpFile := filepath.Join(s.stagingDir, ContentHash([]byte(path)))
